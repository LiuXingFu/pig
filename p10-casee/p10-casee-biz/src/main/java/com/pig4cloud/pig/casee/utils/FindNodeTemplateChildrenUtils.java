...
public class FindNodeTemplateChildrenUtils {
	...
	public static void getGrandParentChildren(List<TaskNodeVO> list, List<TaskNodeVO> voList, String parentId,Integer procedureNature) {
		...
		if (procedureNature.equals(4040)){//程序类型
	...
	private static void getGrandChildrenAssets(List<TaskNodeVO> list, List<TaskNodeVO> voList) {
		for (int i = 0; i < voList.size(); i++) {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if (list.get(j).getNodeAttributes() == 400) {
					 managementVO=(nodeJudgment(list.get(j)));
				}else {
	 ...
	 * @return
	 */
	private static TaskNodeVO nodeJudgment(TaskNodeVO taskNodeVO) {
		if (taskNodeVO.getStatus() == 0) {
			...
			if (taskNodeVO.getNodeKey().equals("entityZX_STZX_CCZXSQYS_CCZXSQYS")||taskNodeVO.getNodeKey().equals("entityZX_STZX_CCZXZCCZYJ_CCZXZCCZYJ")){//商情移送、资产处置移交
				...
				if (taskNodeVO.getNodeKey().equals("entityZX_STZX_CCZXSQYS_CCZXSQYS")){
					if (mortgagee != null && mortgagee == 0 && entityZX_stzx_cczxcf_cczxcf.getSealUpCondition() == 1) {//有抵押权查封为轮候
						return taskNodeVO;
					}
				}else {
					...
					if (entityZX_stzx_cczxcf_cczxcf!=null&&entityZX_stzx_cczxsqys_cczxsqys!=null){
						if ((mortgagee != null && mortgagee == 0 && entityZX_stzx_cczxcf_cczxcf.getSealUpCondition() == 0)||//有抵押权、查封为首封
								(mortgagee != null && mortgagee == 0 && entityZX_stzx_cczxcf_cczxcf.getSealUpCondition() == 1&&entityZX_stzx_cczxsqys_cczxsqys.getBusinessPleaseTransfer()==0)||//有抵押权、查封为轮候、已商情移送(结果成功)
								(mortgagee != null && mortgagee == 1 && entityZX_stzx_cczxcf_cczxcf.getSealUpCondition() == 0&&entityZX_stzx_cczxsqys_cczxsqys.getBusinessPleaseTransfer()==2)) {//无抵押权、首封、无抵押权人
							return taskNodeVO;
						}
					}
				}
